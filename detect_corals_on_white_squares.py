...
Pipeline:
  ...
  Steps 11-13  Within each ROI, segment the coral using the CoralClassifier
               ...
               coral contour.
  Step 14      Detect the 8 calibration circles using detect_calibration_circles.py
               (anchor-pair algorithm; see calibration_README.md for details).
...
from detect_calibration_circles import (
    detect_calibration_circles as _detect_calib_circles,
    PROCESS_SCALE as _CALIB_SCALE,
)
...
# Step 14 calibration circle detection (via detect_calibration_circles.py)
...
# Delegated to detect_calibration_circles.py (anchor-pair algorithm).
# See calibration_README.md for a full description.
...
def process_image(img_path, output_dir):
    ...
    small_cal   = cv2.resize(img, (int(W * _CALIB_SCALE), int(H * _CALIB_SCALE)))
    cal_circles = _detect_calib_circles(small_cal, _CALIB_SCALE)
